...
- [WASM-ENGINE-PORTING.md](./WASM-ENGINE-PORTING.md) — 성능·이식 계획(구상, 1인 개발): WASM/멀티스레드 핵심 엔진 포팅 로드맵(측정→오프메인스레드→수치 커널 WASM→조건부 CA 스캔). "늦기 전에" 진짜 시간제한은 워커 경계뿐이고 물질 89개는 워커 안 JS로 유지(포팅 부채 0)라는 결론.
