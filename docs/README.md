...
- [MATERIAL-ICONS.md](./MATERIAL-ICONS.md) — 물질 아이콘·패턴: 단색 스와치를 캔버스의 실제 질감으로 바꾸는 생성기(`render/materialSvg.ts`) — 렌더러 분기 사슬을 9×9 패치(평면 기체만 18×18 구름 실루엣)에 그대로 재생하고, 셰이딩은 `render/color.ts`로 끌어올려 렌더러와 **한 구현을 공유**한다. 팔레트 126종 전수 분류(패턴 99 / 단색 27), 합성 틴트가 액체의 OU 정상상태 분포를 따라가는 이유, `N = 9`와 색상별 `<path>` 같은 크기·픽셀 정렬 결정, 그리고 **손그림 31종의 발주 목록**(§7 — 규격 자체는 저장소 없이 넘길 수 있는 `create-svg-assets/MATERIAL-ICON-BRIEF.md`). 검증은 `npm run test:materialicons`.
