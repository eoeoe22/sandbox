...
- [MATERIAL-ICONS.md](./MATERIAL-ICONS.md) — 물질 아이콘·패턴: 단색 스와치를 캔버스의 실제 질감으로 바꾸는 생성기(`render/materialSvg.ts`) — 렌더러 분기 사슬을 9×9 패치(평면 기체만 18×18 구름 실루엣)에 그대로 재생하고, 셰이딩은 `render/color.ts`로 끌어올려 렌더러와 **한 구현을 공유**한다. 그 위에 **손그림 13종의 덮어쓰기 계층**(§4.1) — 개념이 정체성이라 유도할 패턴이 없는 물질용이고, `.svg`는 런타임에 로드되지 않고 모듈로 구워진다. 반대 방향도 있다: **Wall은 칩의 줄쌓기 벽돌을 `brickPattern`으로, Woofer는 칩의 스피커 드라이버를 `wooferPattern`으로 캔버스에 되돌렸고**, Obsidian은 무늬 대신 **틴트 진폭을 좁혀**(14 → 6) 유리처럼 매끈해졌다 — 진폭 하나가 캔버스와 아이콘을 같이 좁힌다. 팔레트 126종 전수 분류, 합성 틴트가 액체의 OU 정상상태 분포를 따라가는 이유, `N = 9`와 색상별 `<path>` 같은 크기·픽셀 정렬 결정, 남은 손그림 20종의 발주 목록(§7 — 규격 자체는 저장소 없이 넘길 수 있는 `create-svg-assets/MATERIAL-ICON-BRIEF.md`). 검증은 `npm run test:materialicons` + `npm run check:hand-icons`.
