...
// 아래 넷은 그 규칙(`spoil.ts` 의 `isSubmerged`)의 네 모서리를 각각 하나씩 잡는다.
{
  ...
  //
  // **판별력을 돌연변이로 확인했다**: `sealsSubmersion` 을 항상 false 로(= 벽을 트인
  // 곳으로) 만들면 이 검사가 24 → 12 로 떨어지고, 하네스에서 **이것만** 실패한다.
  // 리뷰에서 「꿀이 세 면에 가까워서 유리 바닥을 잘못 쳐도 통과할 것」이라는 지적이
  // 있었는데, 실제로는 바닥 줄의 가운데 칸들이 `cure` 3~4 대 `open` 1 로 갈린다.
  reseed();
  {
    const { grid, sim } = makeWorld();
