// Bundles and runs the spoil behavioural harness (test/spoil.ts) under Node,
// stubbing the browser-only `?url` wasm import exactly like run-active-tiles.mjs
// does. Run: `node test/run-spoil.mjs`.
import { build } from 'esbuild';
import { fileURLToPath, pathToFileURL } from 'node:url';
import { dirname, join } from 'node:path';

const __dirname = dirname(fileURLToPath(import.meta.url));
const out = join(__dirname, '..', 'node_modules', '.cache', 'spoil.bundle.mjs');
...
await build({
  entryPoints: [join(__dirname, 'spoil.ts')],
  bundle: true,
  platform: 'node',
  format: 'esm',
  outfile: out,
