// Bundles and runs the snapshot file/fit harness (test/snapshot-file.ts) under
...
//
// This harness reaches into src/state/persistence.ts, which pulls in the
...
const out = join(__dirname, '..', 'node_modules', '.cache', 'snapshot-file.bundle.mjs');
...
await build({
  entryPoints: [join(__dirname, 'snapshot-file.ts')],
  bundle: true,
  platform: 'node',
  format: 'esm',
  outfile: out,
