// Bundles and runs the 가이드 데모 harness (test/guidedemo.ts) under Node,
// stubbing the browser-only `?url` wasm import exactly like run-active-tiles.mjs
// does. Run: `node test/run-guidedemo.mjs`.
import { build } from 'esbuild';
import { fileURLToPath, pathToFileURL } from 'node:url';
import { dirname, join } from 'node:path';

const __dirname = dirname(fileURLToPath(import.meta.url));
const out = join(__dirname, '..', 'node_modules', '.cache', 'guidedemo.bundle.mjs');
...
await build({
  entryPoints: [join(__dirname, 'guidedemo.ts')],
  bundle: true,
  platform: 'node',
  format: 'esm',
  outfile: out,
