// Bundles and runs the 캐러멜 harness (test/caramel.ts) under Node,
...
const out = join(__dirname, '..', 'node_modules', '.cache', 'caramel.bundle.mjs');
...
await build({
  entryPoints: [join(__dirname, 'caramel.ts')],
  bundle: true,
  platform: 'node',
  format: 'esm',
  outfile: out,
