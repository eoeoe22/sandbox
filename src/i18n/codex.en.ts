...
export const materialCodexEn: Record<number, string> = {
  ...
  45: 'Water darkens a grain and it cures for a moment before setting into concrete. It stays powder while curing, so water keeps flowing through it, and one cell of water wets several grains on its way down — pour it on top and it soaks deep into the pile.',
  ...
  146: 'A fish that swims through liquid. It drifts loosely toward nearby fish but keeps out of their way, so a tank holds a scattered group rather than a tight shoal. The brush sows only a few at a time. Out of water it flops about and suffocates — not on a fixed clock but with a risk that grows the longer it is stranded, so usually around 12 seconds; heat, blast shockwaves and radiation kill it too. Water conducts, so a current running through a tank electrocutes half the fish in it. It leaves a dead fish that floats to the surface.',
