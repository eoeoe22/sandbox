...
export const materialCodexEn: Record<number, string> = {
  ...
  91: 'Viscous fluid that absorbs water to multiply. Radiation resistant. Current electrolyses it into hydrogen gas rather than back into water, so ground cleared by electricity does not grow back — sweeping a pool with the electric brush is the surest way to be rid of it, and since slime carries current a fair distance, painting the surface reaches the body of the pool. A single spark still only takes a small bite, so clearing a blob outright needs sustained current. Mind the gas: hydrogen is flammable.',
  115: 'A variant formed when slime absorbs acid instead of water, having the same corrosive power as acid. Like acid, it kills any virus it touches — and since it neither drains away nor levels flat, a line of it works as a standing quarantine wall (something plain slime cannot do). Can be diluted by pouring water. It carries current without any loss, so electrolysis takes it apart more readily than plain slime — by the same mechanism, splitting into hydrogen gas rather than reverting to water.',
