...
export const materialCodexEn: Record<number, string> = {
  ...
  159: 'The blotchy grey-green fur that comes up on food once it has started to turn. It grows only into empty space, and only where there is a surface to cling to — so it never blooms underwater, and it films over a surface one cell thick instead of filling a room. It eats nothing and breaks nothing, so a mouldy timber wall is still a wall — but food with mold against it rots three times faster, which is why one spoiled thing in a store room eventually takes the lot. It creeps visibly across food and very slowly across stone or metal. Over 60° or under 0° it dies, and so does touching alcohol, hydrogen peroxide, soapy water or anything acidic.',
  ...
  160: 'What food and corpses leave behind once they have gone over. Raw meat, cooked meat, bread, batter and dead fish all arrive here, and given long enough this breaks down again into compost you can dress a field with. Just lighter than water, so rot in a tank surfaces as scum. It is still rotting itself, so freezing it or laying salt against it holds it as sludge; past 250° it chars to ash. Wet organic matter does not catch fire on its own.',
