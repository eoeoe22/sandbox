<script lang="ts">
  ...
  // directional is selected (or the active tool isn't placing material at all),
  // which is what hides this. Pure readout: pointer-events:none so it can never
  // eat a brush stroke that runs up into the corner.
  const ICONS = {
    up: 'bi-arrow-up',
    down: 'bi-arrow-down',
    left: 'bi-arrow-left',
