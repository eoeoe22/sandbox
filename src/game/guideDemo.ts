 ...
 * 고체 데모에서는 이것이 한 바퀴 전부이고, 겹침 데모에서는 모래와 물 사이에
 * 끼는 한 구간이다(체 선을 긋는다). 두 데모가 **같은 손놀림**을 쓰도록 상수도
 * 대본도 한 벌만 둔다.
 *
 ...
 * 겹침 데모의 체 선 높이(세로 비율).
 *
 * **모래 더미의 봉우리보다 위**여야 한다. 브러시는 빈 칸에만 칠하므로 더미에
 * 걸치면 선이 거기서 끊기는데, 하필 그 자리가 더미 한가운데 — 물줄기가 떨어지는
 * 바로 그 열이라 뚫린 구멍으로 물이 그냥 새 버린다. 처음에 0.55 로 잡았다가
 * 30판 중 여러 판에서 봉우리에 걸리는 것을 보고 내렸다(6초 더미의 봉우리는
 * 최악의 판에서 y=17, 폭 24열).
 *
 * 0.35 이면 56×28 에서 띠가 y=8..12 라 봉우리와 네 줄이 뜬다. 모래 구간이 끝난
 * 뒤에도 알갱이 스무 개쯤이 아직 공중에 있는데, 브러시가 한가운데(x≈28)에
 * 닿는 것은 구간 시작 후 40틱쯤이라 그때는 그 열이 비어 있다.
 */
const MESH_Y = 0.35;
/** 체 선이 걸치는 구간(가로 비율). */
...
export class GuideDemoWorld {
  ...
  private tickOverlap(): void {
    ...
    // 체 선을 긋는 구간. 브러시는 이 구간에만 보인다 — 모래가 떨어지는 동안이나
    // 물이 뿌려지는 동안에도 커서가 떠 있으면 그 손이 그것까지 하는 것으로 읽힌다.
    const mt = t - OVERLAP_SAND;
    if (mt < OVERLAP_MESH) {
      this.dragBrush(mt, Math.round(this.grid.height * MESH_Y), DEMO_MESH.id, MESH_X0, MESH_X1);
      return;
    }
    this.brush = null;
