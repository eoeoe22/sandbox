...
export const GUIDE_DEMO_SPECS: Record<GuideDemoKind, GuideDemoSpec> = {
  ...
  //
  // 화면비에는 **아래쪽 여유**라는 조건이 하나 더 붙는다. 불 덩어리가 파이프
  // 아래로 `PIPE_THICK/2 + FIREBOX_GAP + FIREBOX_H` 줄을 더 먹으므로,
  // `cols / aspect` 로 나오는 세로 칸 수가 그만큼 안 되면 `buildPipe` 가 덩어리를
  // 조용히 잘라 낸다 — 예외도 안 나고 **불만 사라진 열전도 데모**가 된다.
  // 지금 값은 56/3 ≈ 19줄이고 필요한 것은 파이프 한가운데(0.45 → 9줄) 아래로
  // 6줄이다. 이 여유가 깨지면 검사의 「불 Clone 이 덩어리로 놓였다」가 잡는다.
...
export const GUIDE_DEMO_STILL_TICKS: Record<GuideDemoKind, number> = {
  ...
  overlap: 300, // 물이 모래에 스민 뒤 (대본 360틱)
