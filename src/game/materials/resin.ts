...
// Its own 발화점, named because the cure below is gated on it (both the burn and
// the set read the one number) — see `combustion` in the register call.
...
function updateResin(x: number, y: number, sim: SimContext): void {
  ...
  if (sim.getTemp(x, y) < AUTO_IGNITE_TEMP && sim.chance(HARDEN_CHANCE)) {
...
export const RESIN = register({
  ...
  density: 2.6,
  combustion: { burnChance: 0.017, autoIgniteTemp: AUTO_IGNITE_TEMP, easyDouse: true },
  category: 'liquid',
