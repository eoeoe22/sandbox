...
/** aux 0 = crust, aux 1 = crumb. See the note above for why crust is the zero
...
function updateBread(x: number, y: number, sim: SimContext): void {
  // Solid: no fall/flow, so combustion is the only behaviour — if it doesn't
...
export const BREAD = register({
  ...
  // a burnt loaf really is mostly char.
  combustion: { burnChance: 0.05, autoIgniteTemp: 300, ashChance: 0.45 },
  category: 'food',
