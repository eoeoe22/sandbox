...
//     favourites row and in search results (no `Math.random()` here, ever). That
//     synthesis lives in tintNoise.ts, shared with the start screen's title tiles.
//
...
import { varyAmplitude } from '../tint';
import { hex, tinted, buildGlow, shade } from './color';
import { grain, hash8, tintAnchor } from './tintNoise';
