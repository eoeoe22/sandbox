...
import { SHAPED_CHARGE } from '../materials/shapedcharge';
...
export class PointerPainter {
  ...
  private paintCells(cells: readonly number[], erase: boolean): void {
    ...
    // A Fan (blow direction), a Laser (fire direction) and a Shaped Charge (jet
    // direction) record their drag-chosen 상하좌우 direction in the low 2 bits of
    // aux, the rest 0 — see materials/fan.ts, materials/laser.ts and
    // materials/shapedcharge.ts (they share the FAN_* direction codes, so
    // `fanDir` stamps any of them). The Pump used to be stamped here too; it now
    // lifts straight up whatever is inside it, so it has no direction to record
    // and its whole aux byte is the countdown (materials/pump.ts).
    const initAux =
      id === CONVEYOR.id
        ? this.beltDirX < 0
          ? CONVEYOR_LEFT
          : CONVEYOR_RIGHT
        : id === FAN.id || id === LASER.id || id === SHAPED_CHARGE.id
          ? this.fanDir
          : id === CLONE.id
            ? ($cloneTarget.get() ?? 0)
            : 0;
