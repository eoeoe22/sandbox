...
description: 새 물질(파티클) 또는 새 독립 오브젝트를 이 샌드박스에 추가할 때 빠뜨리기 쉬운 전 과정을 순서대로 짚는 체크리스트. id 배정 규칙과 세이브 호환, 등록/카테고리 배선, i18n(한국어명), 인게임 /guide 도감 설명문, 아이콘·스와치 지정, 전기·연소 같은 형질별 부가 등록, 검증 스크립트, docs/ 갱신, Cloudwiki 갱신, 워크플로(PR → 서브에이전트 리뷰 → call_user)까지. "물질 추가", "새 파티클", "오브젝트 추가" 요청이면 코드를 쓰기 전에 읽을 것.
---

# 새 물질 · 오브젝트 추가 체크리스트

물질 하나를 추가하는 데 실제로 손대야 하는 곳은 **코드 5곳 + 문서 4곳**이다. 코드만
넣으면 팔레트에 뜨긴 하지만 한국어명이 빠지고 **인게임 도감에 설명 없이 실린다** —
그리고 한국어명 쪽은 빌드를 실패시키지 않으므로 조용히 새어나간다.

문서를 세 곳으로 가르는 기준은 **읽는 사람**이다(CLAUDE.md 문서 정책). 유저가 읽는
것은 **인게임 도감뿐**이므로 물질 설명은 A-7b 가 정본이고, `docs/`는 작업자용,
Cloudwiki 는 포트폴리오 겸 공유용이다. 같은 문장을 세 곳에 옮겨 적을 필요는 없다.
...
**유저에게 물질을 설명하는 역할은 인게임 도감(A-7b)이 가져갔다.** 위키는 이제
포트폴리오 겸 공유 문서이므로, 물질을 하나 추가할 때마다 갱신할 의무는 없다.
**간판이 될 만한 결과물일 때만** 손대고, 어긋나면 저장소가 맞다.

슬러그로 접근한다(표시 제목 아님).

1. **`Web sandbox 프로젝트/가이드/물질`** (표시 제목 "물질 도감") — 갱신한다면 해당
   `## <이모지> …` 섹션의 표에 행 추가. 문장은 A-7b 의 한국어 설명을 그대로 옮기면
   된다(원본이 그쪽이다).
...
2. **`Web sandbox 프로젝트`** — **간판이 될 만한 기능일 때만** `## 무엇을 하는
   게임인가` 아래 bullet 하나. 편집은 **즉시 반영**을 권장한다.
3. 다단계 공정을 새로 만들었다면 `…/가이드/<공정명>` 자식 페이지 + `…/가이드`의
   `## 공정 가이드`에 `:::card` + `## 플레이 가이드`에 링크.
   템플릿은 `…/가이드/철광석 제련`.
...
12. **`src/i18n/codex.ko.ts`/`codex.en.ts`** — `objectCodexKo`/`objectCodexEn`에
    설명 한 줄씩(`ObjectKind` 키). 유저가 읽는 유일한 설명이다.
    빠지면 `npm run test:codex`가 이름을 대며 실패한다.
13. **Cloudwiki** — 간판급일 때만.
...
4. `docs/` 해당 분야 문서 갱신 + **유저에게 설명이 필요한 변경이면 인게임 도감**
   (`src/i18n/codex.*.ts` / `codexTerms.ts`). Cloudwiki 는 간판급일 때만
...
- `docs/MATERIALS.md` 행 누락 (인게임 도감 쪽은 `test:codex`가 잡아 준다)
