...
재생해 **팔레트 126종 전부의 유도 타일을 만들고, 그중 102종에는 실제 패턴이 붙는다.**
손으로 그리기 전에 그 물질이 이미 패턴을 갖고 있는지 확인할 것 — 스페클·격자·셰브런·
배터리 계단·열 램프·구름은 전부 생성물이고, 손으로 덧그리면 캔버스와 어긋난다.

**손으로 그리기 전에 물어볼 것: 캔버스가 정직한가?** 이 계층에서 가장 자주 나온 실수는
물질 정의의 누락을 그림으로 덮은 것이다. Wood는 손그림으로 나뭇결을 받았지만 인월드의
나무 벽은 여전히 칠한 판자였다 — `colorVary: 16` 한 줄이 양쪽을 같이 고쳤고, 그러자
그림이 필요 없어졌다. 반대로 Mercury·Liquid Gallium은 `colorVary: 0`이 **의도**인
물질이라, 반사선을 그은 칩이 캔버스에 없는 질감을 주장하고 있었다. 셋 다 그림을
**철회**했다. 그림은 유도 타일이 정직하게 틀렸을 때만 값을 한다.

손그림 대상은 **세 부류, 합쳐 29종**이다:

1. **인월드에서도 실제로 평면 한 색인 물질 24종** — 평면 고체 22 + 수은/액체 갈륨(둘은
   단색이 정답이라 **발주 대상이 아니다**). 생성기가 그릴 패턴 자체가 없다.
2. **정체성이 색도 질감도 아니라 개념인 물질** — 패턴이 이미 붙어 있어도 그 패턴이
   물질을 설명하지 못한다. 위 24종 중 6종(Clone·Void·Virus·Catalyst·Amber·
   Aerogel)이 여기 겹치고, 겹치지 않는 건 Fire·Blue Flame(구름)·Antimatter·
   Nanobot(스페클) 4종이다.
3. **생성 결과가 어색하다고 판정된 것 — 태양광 패널 하나.** 4×6 타일이 9셀 창에
   온전히 안 들어가 위아래 셀 높이가 어긋난다.

즉 24 + 4 + 1 = 29. 여기 **들어가지 않는** 것 둘: **칩의 패턴을 캔버스로 되돌린 것**
(Wall·Woofer·TNT — 방향이 반대라 발주가 아니다)과 **카테고리 규칙으로 붙는 표**(방사성
5종의 삼엽 표식 — 물질별 저작물이 아니다). TNT는 그 뒤 한 발 더 가서 **캔버스가 칩을
앞질렀다** — 글자 붙은 종이 띠를 두른 16×16 비트맵 타일이고, **프로젝트에서 글자가 들어간
유일한 그림**이다. 브리프의 글자 금지는 18 px 칩에 대한 것이라 여기에 닿지 않지만,
**칩에는 여전히 글자를 넣지 않는다**(24칸의 4칸 글자는 18 px에서 3 px = 얼룩).

**Wall과 Woofer는 개념 물질이지만 위 세 부류 어디에도 없다** — 둘 다 지금 캔버스 패턴을
갖고 있어서(`brickPattern` / `wooferPattern`) 1번의 "평면 한 색"이 아니고, 그 패턴이 칩에서
온 것이라 마지막 문단의 제외 대상이다. 개념 물질 13종이 이렇게 갈린다: 1번과 겹치는 6 +
2번 4 + 3번 1 + 제외 2. 전수 분류는
...
- **타일 상수는 두 벌인 게 정상이고, 그림은 아니다.** 순회 방식이 달라 코드를 공유할
  수 없으니 숫자는 양쪽에 적고 골든이 대조한다. 하지만 `tntPattern`의 ASCII 16줄처럼
  **어긋나도 눈에 안 보이는 것**은 모듈 하나로 뽑아 공유한다(`render/tntTile.ts`).
  기준은 "공유가 좋은가"가 아니라 "어긋남이 눈에 띄는가"다.
- **틴트 표본 위치도 공유한다.** `Material.tintBlock`이 있으면 셀이 아니라 블록
  앵커(`x & ~(b-1)`, `y & ~(b-1)`)에서 읽는다 — 렌더러와 아이콘이 같은 좌표를 봐야
  한다. 위치 기반이라 **움직이는 알갱이에는 못 쓴다.**
...
- **패치 크기는 픽셀 정렬로 정한다.** 기본 9(18 CSS px에서 셀당 정확히 2 device px
  @DPR1 / 4 @DPR2), 기체·방사성 18(1px / 2px). 12는 DPR 1에서 네 행마다 한 행이
  날아간다. **패치 변은 물질마다 다를 수 있다** — `tntPattern`은 16이다(타일에 글자가
  있어서 9칸 창이 표본이 못 된다). 정렬을 깨는 값이므로, **출하 칩이 손그림인 물질에서만**
  허용된다.
...
받을 때 돌린다. 규격 위반·빈 칸·영역 이탈·기본색 비중을 잡는다(Node, 의존성 없음).

**`node scripts/check-icon-svg.mjs <파일>`** — 저장소에 실행 가능한 파일로 있다.
여기에 사본을 두지 않는다: 두 벌이 되면 갈라지고, 갈라진 쪽을 하필 납품 검수에 쓰게 된다.

반려 사유는 전부 브리프에 글로도 적혀 있어야 한다. **여기에만 있는 규칙은 작성자에게
발견 불가능한 규칙**이다 — 그는 저장소도 스크립트도 없이 브리프만 보고 그린다. 실제로
용량 상한과 중복 속성 금지가 이 상태로 한동안 있었다.

**원본 파일 크기는 검사하지 않는다.** 소비 측이 `<rect>` 목록만 읽고 래퍼를 다시 씌우므로
작성자의 들여쓰기·공백·`<svg>` 속성값은 출하되지 않고, 규격은 그것들을 자유라고 명시한다 —
그래서 어떤 바이트 상한을 걸든 규격을 전부 지킨 파일이 공백만으로 그 위로 넘어갈 수 있다.
출하량은 rect 개수로만 정해지고, 그건 100개로 묶여 있다.
