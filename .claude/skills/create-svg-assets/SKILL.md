...
발주 내용이지 규격이 아니므로, 목록은 `docs/MATERIAL-ICONS.md` §7.3에 두고 작업을
...
않는다. 목록은 `docs/MATERIAL-ICONS.md` §7.3에 있고, 작업을 맡길 때 브리프와 함께
전달한다.
...
| rect 속성이 x/y/width/height/fill 로 고정 | 소비 측 파서가 그 다섯 개만 읽는다 |
