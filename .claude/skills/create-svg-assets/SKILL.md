...
description: 이 샌드박스 프로젝트의 픽셀아트 에셋(독립 오브젝트 스프라이트, 물질 팔레트 아이콘, 캔버스 안에 그려지는 장식)을 만들거나 고칠 때 쓴다. 색 포맷(packed 0xAABBGGRR), 저작 형식 3종(fillRect / ASCII 행 / 벡터 예외), 해상도·종횡비 법칙, 음영·실루엣 하우스 스타일, 팔레트 SVG 내보내기, 크기 예산. 손으로 그리는 물질 아이콘 SVG는 저장소 없이도 쓸 수 있는 자체 완결 발주서(MATERIAL-ICON-BRIEF.md)로 따로 빠져 있어, 외부 도구나 맥락이 끊긴 에이전트에게 그 파일만 통째로 넘기면 된다. "스프라이트 만들어줘", "오브젝트 아트", "물질 아이콘", "SVG 에셋" 같은 요청에서 파일을 열기 전에 읽을 것.
---
...
| 하려는 일 | 읽을 것 | 저장소 필요? |
|---|---|---|
| **물질 아이콘 SVG를 손으로 그린다** | **[MATERIAL-ICON-BRIEF.md](./MATERIAL-ICON-BRIEF.md) 하나면 끝** | ❌ 불필요 |
| 캔버스 안에 들어갈 오브젝트 스프라이트를 만든다 | 이 파일 §1~§6 | ✅ |
| 물질 아이콘 **생성기 코드**를 고친다 | 이 파일 §7 | ✅ |

**`MATERIAL-ICON-BRIEF.md`는 자체 완결 문서다.** 규격·색값·대상 목록·검증 스크립트
까지 전부 그 안에 인라인돼 있어 저장소를 열 필요가 없다. 외부 작업자, 외부 도구,
또는 **이 대화의 맥락을 갖고 있지 않은 서브에이전트에게 그 파일만 통째로 넘기면
된다** — 이 파일이나 `docs/`를 같이 줄 필요 없다.

반대로 **그 브리프를 고칠 때는 자체 완결성을 유지해야 한다.** 새 규칙을 넣으면서
근거를 `CanvasRenderer.ts:912` 같은 참조로만 남기면 받는 쪽에서 확인할 방법이
없어진다. **값은 브리프 안에 적고, 그 값이 어디서 왔는지는 이 파일 §7.1이 기억한다.**
...
손으로 그리는 쪽의 규격·지침·발주 목록은 **[MATERIAL-ICON-BRIEF.md](./MATERIAL-ICON-BRIEF.md)**
...
| 물질별 기본색 | 각 물질 파일의 `register({ color: rgb(...) })` |
| 발주 대상 목록 | 자동 생성이 단색으로 두는 물질 = `docs/MATERIAL-ICONS.md` §4의 B4/B6 |
...
**[MATERIAL-ICON-BRIEF.md](./MATERIAL-ICON-BRIEF.md) §6의 점검표**를 쓴다(검증
...
| `./MATERIAL-ICON-BRIEF.md` | **자체 완결** 손그림 발주서 — 저장소 없이 그대로 넘길 수 있다 |
